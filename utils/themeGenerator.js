...
async function generateTheme(themeConfig) {
    ...
    const themeDirectory = `${buildResultDirectory}/web/wp-content/themes/${slugify(themeConfig.THEME_NAME, { lower: true })}`;
    fs.mkdirSync(themeDirectory, { recursive: true });

    spawnSync("git", ["clone", "git@github.com:gregsullivan/_tw.git", themeDirectory], { stdio: "inherit" });
